...
#### ⑨ OS 언어는 '한국어', 키보드 언어는 'US영어'로 지정하자.
...
#### ⑪ 직접 파티션을 나누기 위해 사용자 레이아웃 만들기 선택
...
#### ⑫ 처음 파티션은 무조건 /boot로 만들어 줘야 한다. 이후 /(root)와 swap도 위와 같이 설정 해주고 다음으로 이동한다.

| CentOS 5 자동 파티션                                 | CentOS 7~ 자동 파티션                                |
| ---------------------------------------------------- | ---------------------------------------------------- |
| /boot   [최소100MB]                                  | /boot     [최소100MB]                                |
| /            [나머지 용량]                           | /             [나머지 용량]                          |
| SWAP   [설정메모리의 2배]                            | /home   [나머지 용량]                                |
|                                                      | SWAP      [설정메모리의 2배]                         |
| 해당 버전의 파티션은 대략적으로 위와 같이 나눠 준다. | 해당 버전의 파티션은 대략적으로 위와 같이 나눠 준다. |
...
#### ⑮ root 계정 최초 비밀번호는 6자리 이상으로 지정하고 다음으로 이동한다.
...
#### ⑰ 다음 버튼을 클릭하면 디스크 여부를 확인 하는데 계속 진행 한다.
...
#### ⑱ 설치가 어느정도 진행 되면 CDROM에 디스크를 2로 교체를 해야 한다. 확인 버튼을 누루지 않고 VirtualBox로 이동한다.
...
#### ㉓ 방화벽의 경우 SSH, Telnet, WWW(HTTP) 정도로만 설정 해준다.
...
#### ㉖ 사용자 생성은 Linux를 사용하면서 만드는 것으로 하기 위해 무시하고 앞으로 이동한다.
